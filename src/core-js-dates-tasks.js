...
function getNextFriday(date) {
  ...
  return new Date(
    date.setDate(date.getDate() + 5 - currentDay + (currentDay >= 5 ? 7 : 0))
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (daysCount > 0) {
    if (
      new Date(year, month - 1, daysCount).getDay() === 6 ||
      new Date(year, month - 1, daysCount).getDay() === 0
    ) {
      hollydays += 1;
    }
    daysCount -= 1;
...
function getWeekNumberByDate(date) {
  const firstMonday = new Date(date.getFullYear(), 0, 4);
  firstMonday.setDate(firstMonday.getDate() - ((firstMonday.getDay() + 6) % 7));

  const daysFromFirstWeek = (date - firstMonday) / 1000 / 60 / 60 / 24;
  return Math.ceil((daysFromFirstWeek + 1) / 7);
}
...
function getQuarter(date) {
  if (date <= new Date(date.getFullYear(), 5, 1)) {
    if (date < new Date(date.getFullYear(), 2, 1)) {
      return 1;
    }
    return 2;
  }
  if (date < new Date(date.getFullYear(), 8, 1)) {
    return 3;
  }
  return 4;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const result = [];
  const arrDataStart = period.start.split('-');
  const arrDataEnd = period.end.split('-');
  ...
  const endData = new Date(arrDataEnd[2], +arrDataEnd[1] - 1, arrDataEnd[0]);
  ...
  while (startData <= endData) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      if (startData <= endData) {
        result.push(
          `${startData.getDate().toString().padStart(2, '0')}-${(startData.getMonth() + 1).toString().padStart(2, '0')}-${startData.getFullYear()}`
        );
      } else {
        break;
      }
    }
    startData.setDate(startData.getDate() + countOffDays);
  }
  return result;
}
